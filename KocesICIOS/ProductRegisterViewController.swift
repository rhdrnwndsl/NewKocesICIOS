...
class ProductRegisterViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate {
    ...
    let usageSegmented = UISegmentedControl(items: ["미사용", "사용"])
    ...
    let defaultImageSegmented = UISegmentedControl(items: ["미사용", "사용"])
...
//        supplyPriceTextField.text = String(conMoney)
    ...
    @objc func saveButtonTapped() {
        ...
        let _count = sqlite.instance.getProductCount()
        if _count > 999 {
            
            let _pDate = Utils.getDate(format: "yyyy-MM-dd HH:mm:ss")
//            let mData = " 데이터 : " + "\n" +
//                    "최대 갯수 제한(1000개)으로 상품 등록 불가";
//            main2Activity.mKocesPosSdk.cout("KocesICApp 환경설정-상품등록",Utils.getLogDate(),"상품등록 " + mData);
            AlertBox(title: "상품등록 실패", message: "상품 최대 갯수 제한(1000)으로 상품 등록 불가", text: "확인")
            return;
        }
        let pSeq = tid +
        Utils.leftPad(str: tableNo, fillChar: "0", length: 3) +
        Utils.leftPad(str: String(_count), fillChar: "0", length: 5)
                        
   
        let pCode = Utils.leftPad(str: String(_count), fillChar: "0", length: 5)
        ...
        if !pVatWon.isNumberByRegularExpression {
            AlertBox(title: "상품등록 실패", message: "부가세액을 숫자만 입력해 주십시오", text: "확인")
            return
        }
        ...
        if !pSvcRate.isNumberByRegularExpression {
            AlertBox(title: "상품등록 실패", message: "봉사료율을 숫자만 입력해 주십시오", text: "확인")
            return
        }
        ...
        if !pSvcWon.isNumberByRegularExpression {
            AlertBox(title: "상품등록 실패", message: "봉사료액을 숫자만 입력해 주십시오", text: "확인")
            return
        }
        guard let pTotalPrice = paymentAmountTextField.text else {
            AlertBox(title: "상품등록 실패", message: "결제금액 합계가 정상적이지 않습니다. 금액 설정을 다시 해주십시오", text: "확인")
            return
        }
        if !pTotalPrice.isNumberByRegularExpression {
            AlertBox(title: "상품등록 실패", message: "결제금액 합계가 숫자만 입력해 주십시오", text: "확인")
            return
        }
        if Int(pVatRate) ?? 0 > 50 {
            AlertBox(title: "상품등록 실패", message: "부가세율은 50%를 넘을 수 없습니다", text: "확인")
            return
        }
        if Int(pSvcRate) ?? 0 > 50 {
            AlertBox(title: "상품등록 실패", message: "봉사료율은 50%를 넘을 수 없습니다", text: "확인")
            return
        }
...
//        if let image2 = productImageView.image,
