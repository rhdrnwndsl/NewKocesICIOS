...
//        case UseCancel
    ...
    @IBOutlet weak var mPointType: UISegmentedControl! //멤버십거래업무구분 사용.M10,조회.M30(취소.M20 사용안함)
    @IBOutlet weak var mPointMoneyView: UIStackView! //구매 또는 취소시 금액을 입력하는 스택. 취소는 제거할 것임
    @IBOutlet weak var mPointTotalView: UIStackView! //구매 또는 취소시 총금액이 입력되는 스택. 취소는 제거할 것임
    ...
    func InitRes() {
        ...
        mPointType.selectedSegmentIndex = 0
        mPointMoneyView.isHidden = false
        mPointMoneyView.alpha = 1.0
        mPointTotalView.isHidden = false
        mPointTotalView.alpha = 1.0
    ...
    func PointTextFieldDidChange(_ textField: UITextField) {
    ...
    @IBAction func segment_point_type(_ sender: UISegmentedControl) {
        switch sender.selectedSegmentIndex {
        case Target.Earn.rawValue:
            mPointMoneyView.isHidden = false
            mPointMoneyView.alpha = 1.0
            mPointTotalView.isHidden = false
            mPointTotalView.alpha = 1.0
            break
        case Target.Use.rawValue:
            mPointMoneyView.isHidden = false
            mPointMoneyView.alpha = 1.0
            mPointTotalView.isHidden = false
            mPointTotalView.alpha = 1.0
        ...
        case Target.Search.rawValue:
            mPointMoneyView.isHidden = true
            mPointMoneyView.alpha = 0.0
            mPointTotalView.isHidden = true
            mPointTotalView.alpha = 0.0
            break
        default:
            break
        }
    ...
    func Point() {
        ...
        if Setting.shared.getDefaultUserData(_key: define.MULTI_STORE) != "" {
            TidAlertBox(title: "거래하실 가맹점을 선택해 주세요") { [self](BSN,TID,NUM,PHONE,OWNER,ADDR) in
                ...
                _tid = TID
                switch mPointType.selectedSegmentIndex {
                case Target.Earn.rawValue:
                    _trdType = Command.CMD_POINT_EARN_REQ
                    break
                case Target.Use.rawValue:
                    _trdType = Command.CMD_POINT_USE_REQ
                    break
                case Target.Search.rawValue:
                    _trdType = Command.CMD_POINT_SEARCH_REQ
                    break
                default:
                    break
                }
                mpaySdk.PointPay(TrdType: _trdType, Tid: _tid, Money: String(money), OriDate: "", CancenInfo: "", mchData: "", CompCode: "", CompName: "", PasswdYN: "", payLinstener: paylistener.delegate!, StoreName: BSN, StoreAddr: ADDR, StoreNumber: NUM, StorePhone: PHONE, StoreOwner: OWNER, Qr: "", IsQr: isCheckCardOrQr.isOn)
            }
            return
        }
        ...
        mpaySdk.PointPay(TrdType: _trdType, Tid: _tid, Money: String(money), OriDate: "", CancenInfo: "", mchData: "", CompCode: "", CompName: "", PasswdYN: "", payLinstener: paylistener.delegate!, StoreName: Setting.shared.getDefaultUserData(_key: define.STORE_NAME),StoreAddr: Setting.shared.getDefaultUserData(_key: define.STORE_ADDR),StoreNumber: Setting.shared.getDefaultUserData(_key: define.STORE_BSN),StorePhone: Setting.shared.getDefaultUserData(_key: define.STORE_PHONE),StoreOwner: Setting.shared.getDefaultUserData(_key: define.STORE_OWNER), Qr: "", IsQr: isCheckCardOrQr.isOn)
        
    }
    
}
extension PointController : UITabBarControllerDelegate, PayResultDelegate, UITextFieldDelegate {
    func onPaymentResult(payTitle _status: payStatus, payResult _message: Dictionary<String, String>) {
        ...
        var _title:String = "포인트[불가]"          //타이틀
        var keyCount:Int = 0
        switch _message["TrdType"] {
        case Command.CMD_POINT_USE_RES, Command.CMD_POINT_EARN_RES:
            _title = "포인트[거래]"
            ...
            break
        case Command.CMD_POINT_USE_CANCEL_RES, Command.CMD_POINT_EARN_CANCEL_RES:
            _title = "포인트[취소]"
            for (key,value) in _message {
                if _message.count - 1 == keyCount {
                    _totalString += key + "=" + value
        ...
        if _status == .OK {
            if _message["TrdType"] == Command.CMD_POINT_SEARCH_RES {
            ...
            controller.rootView.setData(영수증데이터: sqlite.instance.getTradeLastData(), 뷰컨트롤러: "포인트", 전표번호: String(sqlite.instance.getTradeList().count))
...
extension PointController: NumberPadDelegate {
    func keyPressed(key: NumberKey?) {
        ...
        case .delete:
            ...
            case "Money":
                ...
                //                    TxtUpdate(IsNext: false)
                //                }
                PointTextFieldDidChange(mMoney)
                break
            default:
                break
            }
...
//                TxtUpdate(IsNext: false)
        ...
        case .keyok:
            switch mPointType.selectedSegmentIndex {
            case Target.Use.rawValue, Target.Earn.rawValue:
                if mMoney.text == "" {
